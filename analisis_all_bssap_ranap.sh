...
then
	...
	temp=$(tshark -V -Y "( ( gsm_a.dtap.msg_mm_type == 0x19 ) || ( gsm_a.bssmap.msgtype == 0x55 ) ) && ( gsm_a.imeisv contains $5 )" -r "$dir_unlzma"${2:0:14} 2>/dev/null)
	c_f_s=$(echo "$temp" | grep -c -e "BCD Digits: $5")

	if [ $c_f_s -gt '0' ]
	then

		f_s=$(echo "$temp" | grep -n -e "BCD Digits: $5")
